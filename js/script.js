...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const declOfNum = (n, titles, from) => n + ' ' + titles[from ? n % 10 === 1 && n % 100 !== 11 ? 1 : 2 : 
    ...
    const renderResponse = response => {
        if  (response.ok)  {
            ...
            cardHead.style.color = '#1C1C1C';
            cardHead.style.backgroundColor = '#54FF9F';
            cardHead.style.borderRadius = '5px';
        }
            setTimeout( 
                () => {
                    location.reload(); 
                    document.querySelector('.form-calculate').reset();
                }, 5000);
    };
